...
// グローバル変数
// ==============================
int count;			// ※※　整数型(int)の変数count						// 推測回数	
int cursor;			// ※※　整数型(int)の変数cursor					// 選択カーソル
int target[DIGITS];	// ※※　整数型(int)、配列の要素数DIGITSのtarget	// 目標数の保存配列
int num[DIGITS];	// ※※　整数型(int)、配列の要素数DIGITSのnum		// 推測数の保存配列
		...
		if(gameClear == false)
		{
			...
			else if(IsPushKey(MY_INPUT_ENTER))
			{
				...
				if(IsValidNumber() == true)
				{
					...
					IsMatch(&hit, &blow);	// hit, blowの数をチェックし、その返り値を gameClear に代入
					if (hit == DIGITS)
					{
						gameClear = true;
					}
